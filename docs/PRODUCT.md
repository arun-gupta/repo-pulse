...
- The org-summary strip also includes a **Recommendations** tab positioned after Security (issue #359). It aggregates the per-repo recommendation catalog into *top systemic issues* across the analyzed repo set, grouped by CHAOSS dimension (Activity, Responsiveness, Contributors, Documentation, Security), sorted by the number of affected repos. Each aggregated entry expands to show the alphabetical list of affected repo slugs for direct triage. The tab reuses the per-repo recommendation catalog as its source of truth — no new recommendation rules are introduced.
