...
The calibration script (`calibrate`) uses three API calls per repo:
...
```json
{
  ...
  "source": "GitHub Search API + lightweight GraphQL (calibrate)",
  "sampleSizes": {
    "emerging": 51,
    "growing": 52,
    "established": 52,
...
The script checkpoints to `scripts/calibrate-checkpoint.json` after every batch. If interrupted, re-running resumes from where it left off. Delete the checkpoint file to start fresh.
